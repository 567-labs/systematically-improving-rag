...
**Real Impact**: Fine-tuning delivers 6-10% improvements that compound with other optimizations. Combined with better evaluation (Chapter 1), teams see 15-20% total gains.

<!--
Key message: We're not fine-tuning language models (those are expensive and hard). We're fine-tuning embedding models - much easier and cheaper. The goal is to improve retrieval, not generation. This is the fundamental shift companies need to make. Fine-tuning embedding models with just 6,000 examples can deliver measurable improvements in 40 minutes on a laptop.
-->
